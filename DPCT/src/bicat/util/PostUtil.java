...
public class PostUtil {
	...
	// ===========================================================================
	static LinkedList filterBySize(int b, int g, int c, LinkedList list_in) {
	...
	// ===========================================================================
	static LinkedList filterByNoOverlap(int K, int thr, LinkedList list) {
		...
		for (int i = 1; i < list.size(); i++) {
			...
			if (next.getGenesSize() == 1 || next.getChipsSize() == 1)
				continue; // avoid singletons (mislim da je svakako prije
							// uradjeno!)
	...
	// ===========================================================================
	public static LinkedList filterByNewArea(int b, int t, LinkedList list,
			int N, int M) {
	...
	public static LinkedList filterByHammingDistance(int err, LinkedList list,
	...
	// ===========================================================================
	private static LinkedList clean_list_of_bcs(LinkedList list) {
	...
	// ===========================================================================
	private static int computeMatrixCoverage(int[][] bm, Bicluster next) {
		int na = 0;
		for (int i = 0; i < next.genes.length; i++)
			for (int j = 0; j < next.chips.length; j++)
				if (bm[i][j] == 0)
					na++;
		return na;
	}
	...
	// ===========================================================================
	private static int[][] updateMatrix(int[][] bm, Bicluster next) {
		for (int i = 0; i < next.genes.length; i++)
			for (int j = 0; j < next.chips.length; j++)
				if (bm[i][j] == 0)
					bm[i][j] = 1;
	...
	// ===========================================================================
	// ....
	public static LinkedList search(int minSize, int minGeneCount,
		...
		for (int i = 0; i < geneNames.length; i++) { // get indices of the
			...
			if (!geneNames[i].equals("")) {
				int index = BicatGui.currentDataset.getGeneIdx(geneNames[i]); // pre.getGeneIndex(geneNames[i]);
		...
		for (int i = 0; i < chipNames.length; i++) {
			if (!chipNames[i].equals("")) {
				int index = BicatGui.currentDataset
			...
			for (int i = 0; i < list.size(); i++) {
				...
				for (int p = 0; p < geneIndices.size(); p++)
					for (int q = 0; q < next.getGenesSize(); q++) {
						...
						if (next.genes[q] == ((Integer) geneIndices.get(p))
								.intValue()) {
							cnt++;
							break;
						}
				...
				for (int p = 0; p < chipIndices.size(); p++)
					for (int q = 0; q < next.getChipsSize(); q++)
						if (next.chips[q] == ((Integer) chipIndices.get(p))
								.intValue()) {
							cnt++;
							break;
						}
	...
	// ===========================================================================
	static public Bicluster getHammingDistanceExtension(Bicluster bc,
		...
		for (int i = 0; i < bc.genes.length; i++)
			bc_gene_side.set(bc.genes[i]);
		for (int i = 0; i < bc.chips.length; i++)
			bc_chip_side.set(bc.chips[i]);
		...
		double err_perc = (double) ((double) err / (double) 100);
		...
		for (int i = 0; i < chip_nummer; i++) {
			if (bc_chip_side.get(i) == false) { // that star is not already in a
												// bc
			...
			if (best_genes.size() > 0) {
				...
				if (((Gene) best_genes.get(0)).hd_err < min_hd) {
					...
					if (best_genes.size() > 0) {
						...
						double up = (double) ((double) (hd + ext_bc.error) / (double) ((bc_gene_side
						...
						if (up < err_perc) {
							...
							break;
						up = (double) (((double) ext_bc.error) / ((double) (bc_gene_side
					...
					if (best_chips.size() > 0) {
						...
						if (up < err_perc) {
							...
							ext_bc.size += bc_gene_side.cardinality();
