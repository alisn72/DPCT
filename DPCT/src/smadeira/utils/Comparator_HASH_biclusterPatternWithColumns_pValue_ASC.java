 ...
 *           bicluster pattern with columns p-value in ascending order.</p>
 *
 * <p>Description: This <code>Comparator</code> stores an hash table with
 *                 the bicluster pattern with columns p-value for each bicluster
 ...
 *                 allows to sort biclusters by their pattern with columns
 ...
 * @version 1.0
 */
public class Comparator_HASH_biclusterPatternWithColumns_pValue_ASC
    extends AbstractComparator_HASH<Double> {
  /**
   * Creates a new <code>Comparator</code> which stores an hash table with
   * the pattern with columns p-value for each bicluster in
   ...
   */
  public Comparator_HASH_biclusterPatternWithColumns_pValue_ASC(
      BiclusteringInDiscretizedMatrix biclustering,
      int markovChainOrder) {
    super(biclustering);
    ...
    ArrayList<Bicluster> biclusters = biclustering.getBiclusters();
      ...
      for (Bicluster bicluster : biclusters) {
        metrics.put(bicluster.getID(),
                    ...
                    getBiclusterPatternWithColumns_pValue(markovChainOrder));
   ...
   * bicluster pattern with columns p-value of the first bicluster is
   * less than, equal to, or greater than the bicluster pattern with
   * columns p-value of the second.
   *
