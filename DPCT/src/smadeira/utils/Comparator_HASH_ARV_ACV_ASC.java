 ...
 * @version 1.0
 */
public class Comparator_HASH_ARV_ACV_ASC
    extends AbstractComparator_HASH<Float> {
  /**
   * Creates a new <code>Comparator</code> which stores an hash table with
   * the value of ARV-ACV for each bicluster in <code>biclustering</code>.
   *
   * @param biclustering the group of biclusters to initialize the
   *        metric table from
   */
  public Comparator_HASH_ARV_ACV_ASC(Biclustering biclustering) {
    super(biclustering);
    ...
    for (Bicluster bicluster : biclusters) {
      metrics.put(Integer.valueOf(bicluster.getID()),
                  bicluster.computeARV() - bicluster.computeACV());
   ...
   * than the ARV-ACV of the second.
   *
