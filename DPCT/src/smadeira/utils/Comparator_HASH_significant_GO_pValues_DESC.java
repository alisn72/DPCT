 ...
 *           number of significant GO p-values in descending order.</p>
 *
 * <p>Description: This <code>Comparator</code> stores an hash table with
 *                 the number of significant GO p-values for each
 ...
 * @version 1.0
 */
public class Comparator_HASH_significant_GO_pValues_DESC
    extends AbstractComparator_HASH<Integer> {
  /**
   * Creates a new <code>Comparator</code> which stores an hash table with
   * the number of significant GO p-values for each bicluster in
   ...
   * @param go GOFrontEnd
   */
  public Comparator_HASH_significant_GO_pValues_DESC(
    ...
    for (Bicluster bicluster : biclusters) {
      ...
      metrics.put(bicluster.getID(),
                  bicluster.computeSignificant_GO_pValues(result));
   ...
   * number of significant GO p-values of the first bicluster is
   * greater than, equal to, or less than the number of significant
   * GO p-values of the second.
   *
