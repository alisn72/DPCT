 ...
 *           number of genes in descending order.</p>
 *
 ...
 * @version 1.0
 */
public class Comparator_HASH_numberOfGenes_DESC
    extends Comparator_HASH_numberOfGenes_ASC {
  /**
   * Creates a new <code>Comparator</code> which stores an hash table with
   * the number of genes for each bicluster in
   * <code>biclustering</code>.
   *
   * @param biclustering the group of biclusters to initialize the
   *        metric table from
   */
  public Comparator_HASH_numberOfGenes_DESC(Biclustering biclustering) {
    super(biclustering);
  }
   ...
   * number of genes of the first bicluster is greater than, equal to, or less
   * than the number of genes of the second.
   *
