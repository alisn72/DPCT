 ...
 *           number of significant GO p-values for a given p-value user
 *           threshold in descending order.</p>
 *
 * <p>Description: This <code>Comparator</code> stores an hash table with
 *                 the number of significant GO
 *                 p-values for a given p-value user threshold
 *                 for each bicluster in a group of biclusters.
 ...
 *                 biclusters by their number of significant GO p-values for
 *                 a given p-value user threshold in descending order.
 * </p>
 *
 ...
 * @version 1.0
 */
public class Comparator_HASH_significant_GO_pValues_userThreshold_DESC
    extends AbstractComparator_HASH<Integer> {
  /**
   * Creates a new <code>Comparator</code> which stores an hash table with
   * the number of significant GO p-values for a given p-value user
   * threshold for each bicluster in <code>biclustering</code>.
   *
   ...
   */
  public Comparator_HASH_significant_GO_pValues_userThreshold_DESC(
      Biclustering biclustering, GOFrontEnd go, float GO_pValues_userThreshold) {
    super(biclustering);
    ...
    for (Bicluster bicluster : biclusters) {
      ...
      metrics.put(bicluster.getID(),
                  bicluster.computeSignificant_GO_pValues_UserThreshold(result,
   ...
   * number of significant GO p-values for a given p-value user threshold of
   * the first bicluster is greater than, equal to, or less than the
   * number of significant GO p-values for a given p-value user threshold
   * of the second.
   *
