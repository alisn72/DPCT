...
/**
 * <p>Title: Suffix Tree No Errors Builder - STEP 2</p>
 *
 * <p>Description: Builder of a Suffix Tree extended for identifying
 *                 biclusters with exact patterns (CCC Biclusters)
 *                 and special characteristics,
 *                 such as gene shifts (scaled patterns), sign changes
 *                 (anticorrelated patterns) or time lags (time lagged
 *                 patterns) - STEP 2.</p>
 *
 ...
 * @version 1.0
 */
public class SuffixTreeNoErrors_Extended_STEP2_Builder
    extends SuffixTreeNoErrorsBuilder {

  /**
   * Objects of the class InternalNodeNoErrors_Extended are created with:
   * isMaximal = false
   *
   *
   * WHEN THE ROWS WITH EXTENSIONS (SIGN-CHANGES OR GENE-SHIFTS ...) ARE ADDED TO THE TREE, THE NEW NODES CREATED
   * MUST BE MARKED AS NON-MAXIMAL (BEFORE EXTENSIONS) SINCE THEY DID NOT EXIST YET
   */
   ...
   */
  public SuffixTreeNoErrors_Extended_STEP2_Builder(SuffixTreeNoErrors_Extended
      tree) {
    super(tree);
  }
  ...
  protected boolean extend(int phase, int extension) {
    ...
    if (gammaLen == 0) {
       ...
       */
      NodeInterface temp = ( (InternalNodeNoErrors_Extended)this.current.
                            getLast()).getFirstChild();
      if (temp == null) {
        LeafNodeNoErrors t = new LeafNodeNoErrors(this.startPos, this.tokenLen, null);
        ( (InternalNodeNoErrors_Extended)this.current.getLast()).setFirstChild(
            t);
        isExplicit = true;
      }
      else {
        ...
        if (startP == phaseChar) { // REGRA 3
          ...
          if (phaseChar < startP) {
            ...
            if (tempP instanceof LeafNodeNoErrors) {
              ...
              ( (InternalNodeNoErrors_Extended) tempP).setRightSybling(t);
            }
          }
          else {
            /**
            ...
            ( (InternalNodeNoErrors_Extended)this.current.getLast()).
                setFirstChild(t);
          }
          isExplicit = true;
        }
      ...
      if (nextChar == phaseChar) { //REGRA 3
        ...
        //!!!
        InternalNodeNoErrors_Extended tempI = new InternalNodeNoErrors_Extended(
            down.getLeftIndex(), gammaLen, down.getRightSybling(), null);
        //!!!
        //VERIFY THAT THIS IS NOT NEEDED HERE AND IN FACT CANT MARK AS NON MAXIMAL BICLUSTERS THAR ARE MAXIMAL
        //        tempI.setIsMaximal(false);

        if (down instanceof InternalNodeNoErrors_Extended) {
          ( (InternalNodeNoErrors_Extended) down).setLeftIndex(down.
              getLeftIndex() + gammaLen);
          ( (InternalNodeNoErrors_Extended) down).setLength(down.getLength() -
              gammaLen);
          ( (InternalNodeNoErrors_Extended) down).setRightSybling(null);
        }
        else {
        ...
        if (this.current.size() > 2) {
          ...
          if (removed instanceof InternalNodeNoErrors_Extended) {
            ( (InternalNodeNoErrors_Extended) (removed)).setRightSybling(tempI);
          }
          else {
          ...
          ( (InternalNodeNoErrors_Extended)this.current.getFirst()).
              setFirstChild(tempI);
        }
        /**
    ...
    if ( (this.previous.size() != 0) &&
        (previous.getLast() instanceof InternalNodeNoErrors_Extended)) {
      if ( ( ( (InternalNodeNoErrors_Extended) previous.getLast()).
            getSuffixLink() == null) &&
          ( (InternalNodeNoErrors_Extended)this.previous.getLast() !=
           this.myTree.getRoot())) {
        ( (InternalNodeNoErrors_Extended)this.previous.getLast()).setSuffixLink( (
            InternalNodeNoErrors_Extended)this.current.getLast());
      }
    }
