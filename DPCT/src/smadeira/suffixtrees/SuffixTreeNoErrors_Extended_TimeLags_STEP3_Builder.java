 ...
 * @version 1.0
 */
public class SuffixTreeNoErrors_Extended_TimeLags_STEP3_Builder
    extends SuffixTreeNoErrorsBuilder {

  /**
   * Objects of the class InternalNodeNoErrors_Extended_TimeLags are created with:
   * isMaximal = false
   * isMaximal_After_Extensions = false
   *
   * WHEN THE ROWS WITH TIME-LAGS ARE ADDED TO THE TREE, THE NEW NODES CREATED
   * MUST BE MARKED AS NON-MAXIMAL (BEFORE EXTENSIONS) SINCE THEY DID NOT EXIST YET
   * AND AS SUCH COULD NOT BE MAXIMAL
   */
   ...
   */
  public SuffixTreeNoErrors_Extended_TimeLags_STEP3_Builder(
  ...
  protected boolean extend(int phase, int extension) {
    ...
    if (gammaLen == 0) {
      ...
      if (nextChar == phaseChar) { //REGRA 3
        ...
        tempI.setIsMaximal_After_Extensions(false);
        //!!!
