 ...
 * @version 1.0
 */
public class SuffixTreeNoErrors_TimeLags_STEP2_Builder
    extends SuffixTreeNoErrorsBuilder {

  /**
   *
   * Objects of the class SuffixTreeNoErrors_TimeLags are created with
   * isMaximal = false
   * isMaximal_After_TimeLags = true
   *
   * WHEN THE ROWS WITH TIMELAGS ARE ADDED TO THE TREE, THE NEW NODES CREATED
   * MUST BE MARKED AS NON-MAXIMAL (BEFORE TIME-LAGS) SINCE THEY DID NOT EXIST YET
   */
   ...
   */
  public SuffixTreeNoErrors_TimeLags_STEP2_Builder(SuffixTreeNoErrors_TimeLags
      tree) {
    super(tree);
  }
  ...
  protected boolean extend(int phase, int extension) {
    ...
    if (gammaLen == 0) {
       ...
       */
      NodeInterface temp = ( (InternalNodeNoErrors_TimeLags)this.current.
                            getLast()).getFirstChild();
      if (temp == null) {
        LeafNodeNoErrors t = new LeafNodeNoErrors(this.startPos, this.tokenLen, null);
        ( (InternalNodeNoErrors_TimeLags)this.current.getLast()).setFirstChild(
            t);
        isExplicit = true;
      }
      else {
        ...
        if (startP == phaseChar) { // REGRA 3
          ...
          if (phaseChar < startP) {
            ...
            if (tempP instanceof LeafNodeNoErrors) {
              ...
              ( (InternalNodeNoErrors_TimeLags) tempP).setRightSybling(t);
            }
          }
          else {
            /**
            ...
            ( (InternalNodeNoErrors_TimeLags)this.current.getLast()).
                setFirstChild(t);
          }
          isExplicit = true;
        }
      ...
      if (nextChar == phaseChar) { //REGRA 3
         ...
         */
        InternalNodeNoErrors_TimeLags tempI = new InternalNodeNoErrors_TimeLags(
            down.getLeftIndex(), gammaLen, down.getRightSybling(), null);
        //!!!
        // THIS NODE IS CREATED WITH THE TIME-LAGS ADDED TO THE TREE -> IT COULD NOT BE MAXIMAL BEFORE TIME-LAGS SINCE IT DIDN´T EXIST
        tempI.setIsMaximal(false);

        if (down instanceof InternalNodeNoErrors_TimeLags) {
          ( (InternalNodeNoErrors_TimeLags) down).setLeftIndex(down.
              getLeftIndex() + gammaLen);
          ( (InternalNodeNoErrors_TimeLags) down).setLength(down.getLength() -
              gammaLen);
          ( (InternalNodeNoErrors_TimeLags) down).setRightSybling(null);
        }
        else {
        ...
        if (this.current.size() > 2) {
          ...
          if (removed instanceof InternalNodeNoErrors_TimeLags) {
            ( (InternalNodeNoErrors_TimeLags) (removed)).setRightSybling(tempI);
          }
          else {
          ...
          ( (InternalNodeNoErrors_TimeLags)this.current.getFirst()).
              setFirstChild(tempI);
        }
        /**
    ...
    if ( (this.previous.size() != 0) &&
        (previous.getLast() instanceof InternalNodeNoErrors_TimeLags)) {
      if ( ( ( (InternalNodeNoErrors_TimeLags) previous.getLast()).
            getSuffixLink() == null) &&
          ( (InternalNodeNoErrors_TimeLags)this.previous.getLast() !=
           this.myTree.getRoot())) {
        ( (InternalNodeNoErrors_TimeLags)this.previous.getLast()).setSuffixLink( (
            InternalNodeNoErrors_TimeLags)this.current.getLast());
      }
    }
