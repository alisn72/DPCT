 ...
 */
package math3.stat.correlation;

import math3.stat.regression.SimpleRegression;
...
public class PearsonsCorrelation {
    ...
    public RealMatrix computeCorrelationMatrix(RealMatrix matrix) {
        ...
        for (int i = 0; i < nVars; i++) {
            for (int j = 0; j < i; j++) {
              double corr = correlation(matrix.getColumn(i), matrix.getColumn(j));/*Spearman_correlation(matrix.getColumn(i), matrix.getColumn(j));Reflective_correlation(matrix.getColumn(i), matrix.getColumn(j));*/
    ...
    public double Spearman_correlation(final double[] xArray, final double[] yArray) {
        SimpleRegression regression = new SimpleRegression();
        double sum = 0;
        if (xArray.length != yArray.length) {
            throw new DimensionMismatchException(xArray.length, yArray.length);
            ...
            double[] x_ranked = ranker(xArray);
            double[] y_ranked = ranker(yArray);
            for(int i=0;i<x_ranked.length;i++){
                sum += Math.pow(Math.abs(x_ranked[i] - y_ranked[i]), 2);
            }
        }
        return ( 1-(6*sum / ( xArray.length*Math.pow(xArray.length,2)-1)));
    }
    public double Reflective_correlation(final double[] xArray, final double[] yArray) {
        ...
        if (xArray.length != yArray.length) {
            ...
            for(int i=0;i<xArray.length;i++) {
                sumX += Math.pow(xArray[i],2);
                sumY += Math.pow(yArray[i],2);
    ...
    private double[] ranker(final double[] input) {
        double[] sorted = input;
        double temp;
        ...
        double[] rankes = new double[input.length];
