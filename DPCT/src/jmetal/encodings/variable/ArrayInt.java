...
public class ArrayInt extends Variable {
  ...
  public ArrayInt(int numberOfGeneVariables_, int numberOfConditionVariables_, Problem problem) {
    problem_ = problem ;
    size_   = numberOfGeneVariables_+numberOfConditionVariables_;
    array_ = new int[size_];
    lowerBounds_ = new int[size_] ;
    upperBounds_ = new int[size_] ;
    ...
    for (int i = 0; i < size_ ; i++) {
      lowerBounds_[i] = (int)problem_.getLowerLimit(i);
      upperBounds_[i] = (int)problem_.getUpperLimit(i);
      if(i<numberOfGeneVariables_)
          array_[i] = PseudoRandom.randInt(0, problem_.geneExpressionData.sizeOfProteins-1);
      else
          array_[i] = PseudoRandom.randInt(0, problem_.geneExpressionData.sizeOfConditions-1);
