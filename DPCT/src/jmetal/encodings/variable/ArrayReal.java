...
public class ArrayReal extends Variable {
  ...
  public ArrayReal(int numberOfGeneVariables_, int numberOfConditionVariables_, Problem problem) {
    problem_ = problem ;
    size_   = problem_.numberOfVariables_;
    array_ = new Double[size_];

    for (int i = 0; i < size_ ; i++) {
      if( (i%(numberOfGeneVariables_+numberOfConditionVariables_)) < numberOfGeneVariables_ )
        array_[i] = (double)PseudoRandom.randInt(0, problem_.geneExpressionData.sizeOfProteins-1);
      else
        array_[i] = (double)PseudoRandom.randInt(0, problem_.geneExpressionData.sizeOfConditions - 1);

    }

  }

    public ArrayReal(int sizeOfGenePart, int sizeOfConditionPart, int numberOfVariables_, Problem problem) {
        problem_ = problem ;
        size_   = numberOfVariables_;
        array_ = new Double[size_];

        for (int i = 0; i < size_ ; i++) {
            int c = i%(sizeOfGenePart+sizeOfConditionPart);
            if( c < sizeOfGenePart )
                array_[i] = (double)PseudoRandom.randInt(-1, problem_.geneExpressionData.sizeOfProteins-1);
            else
                array_[i] = (double)PseudoRandom.randInt(-1, problem_.geneExpressionData.sizeOfConditions - 1);
