...
public class SolutionSet implements Serializable {
  ...
  */
  ...
  public void printVariablesToFile(String path){
    try {
      ...
      if (size()>0) {
        ...
        bw.write(solutionsList_.get(0).getProblem().sizeOfGenePart+"\n");
        bw.write(solutionsList_.get(0).getProblem().sizeOfConditionPart+"\n");
  ...
  public void printVariablesToFile(String path, int sizeOfGenePart, int sizeOfConditionPart) {
    try {
      FileOutputStream fos   = new FileOutputStream(path)     ;
      OutputStreamWriter osw = new OutputStreamWriter(fos)    ;
      BufferedWriter bw      = new BufferedWriter(osw)        ;
      ...
      if (size()>0) {
        bw.write(sizeOfGenePart+"\n");
          bw.write(sizeOfConditionPart+"\n");
        int numberOfVariables = solutionsList_.get(0).getDecisionVariables().length ;
        for (Solution aSolutionsList_ : solutionsList_) {
          for (int j = 0; j < numberOfVariables; j++)
            bw.write(aSolutionsList_.getDecisionVariables()[j].toString() + " ");
